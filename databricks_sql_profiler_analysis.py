...
- **テキストベースの記述**: すべての絵文字を削除し、プロフェッショナルなテキストベース記述に変更してください
...
- **見出しレベルの厳格な階層構造**: 
  - # レベル1: メインタイトル
  - ## レベル2: 主要セクション (1. ボトルネック分析結果)
  - ### レベル3: サブセクション (AIによる詳細分析)
  - #### レベル4: 詳細項目 ((1) 主要ボトルネックと原因)
- **番号付けの統一**: 
  - レベル2: "1. 2. 3." を使用
  - レベル4: "(1) (2) (3)" を使用して区別
- **TOP10処理の表示**: <br>タグを使用して改行を明確化し、絵文字を削除して読みやすいテキスト形式で整理
- **セクション区切り**: --- を使用してセクションを明確に区切る
- **プロフェッショナルな構造**: 統一されたインデントと段落間隔を維持
...
5. **一貫性の確保**: 用語や表記の統一
6. **見出し構造の修正**: 見出しレベルを適切に整理し、階層を統一
7. **Broadcast説明の簡潔化**: Broadcastに関する説明は簡潔にまとめる
8. **TOP10表示の改善**: <br>タグを使用して改行を明確化し、読みやすくする
9. **絵文字の削除**: すべての絵文字を削除し、プロフェッショナルなテキストに変更
10. **区切り文字の削除**: =========等の区切り文字を削除
11. **冗長セクションの統合**: Liquid Clustering関連の重複セクションを統合
